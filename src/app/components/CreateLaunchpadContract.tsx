...
export default function CreateLaunchPadContract({
	...
	const { data } = useSimulateContract({
		...
		args: [
			name,
			maxCap,
			Math.round(new Date(startAt).getTime() / 1000),
			Math.round(new Date(endAt).getTime() / 1000),
			noOfTier,
			projectOwner,
			tokenAddress,
			1,
	...
	return (
		<div>
			<button className="btn w-60 btn-accent" onClick={() => handleOpenModal()}>
			...
			<dialog id="create_launchpad_contract" className="modal">
				<div className="modal-box w-11/12 max-w-5xl flex flex-col gap-4">
					...
					<label className="form-control w-full max-w">
						<div className="label">
							<span className="label-text">
								What is your owner wallet address?
							</span>
						</div>
						<input
							value={projectOwner}
					...
					<label className="form-control w-full max-w">
						...
						<input
							value={noOfTier}
							onChange={(e) => setNoOfTier(Number(e.target.value))}
							type="number"
							className="input input-bordered w-full max-w"
						/>
					</label>
					...
					<div className="modal-action">
						<button
							disabled={!Boolean(data?.request) || launchPadContract.length > 0}
							onClick={() =>
